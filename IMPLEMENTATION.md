...
- [Setup](#setup) · [Running locally](#running-locally) · [Continuous integration](#continuous-integration)
- [Operating it](#clearing-the-database): clearing data, inspecting a running bot
- Why it is the way it is: [the weekly window](#the-weekly-window-is-not-reconfigurable),
  [the all-time image](#the-all-time-image-is-off-by-default),
  [dependencies](#dependencies), [layout](#layout),
  [deviations from the design](#where-this-deviates-from-the-design-document)
...
[TESTPLAN.md](TESTPLAN.md) is a ten-minute walkthrough for the live bot using throwaway
...
**Back it up first.** The session log is the one thing nothing else can rebuild:
...
For a handful of rows, the bot is enough: `/undo` removes the most recent
session each time, and `/delete <name>` removes an objective once nothing is
recorded against it.

To wipe everything at once — the schema and the three `config` rows survive:
...
The `sqlite_sequence` line is optional; it just makes the next tree start at
id 1 rather than continuing the previous numbering.

Multi-statement `--command` works. If it fails with `code: 7403`, see
[If a remote D1 command fails](#if-a-remote-d1-command-fails-with-code-7403).
...
`wrangler tail` is also how to confirm the CPU figures in [The all-time image
is off by default](#the-all-time-image-is-off-by-default) on a real
